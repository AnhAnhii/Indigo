...
import { CheckSquare, Utensils, Users, MapPin, CheckCircle2, AlertCircle, ChefHat, Plus, Trash2, ListChecks, Square } from 'lucide-react';
import { useGlobalContext } from '../contexts/GlobalContext';
import { PrepTask } from '../types';

export const KitchenView: React.FC = () => {
  const { servingGroups, toggleSauceItem, prepTasks, addPrepTask, togglePrepTask, deletePrepTask, currentUser } = useGlobalContext();
    ...
    <div className="space-y-6 pb-20">
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500">Quản lý đồ dùng, nước chấm và công việc nội bộ.</p>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* LEFT COLUMN: STANDALONE TASKS (NEW FEATURE) */}
          <div className="lg:col-span-1 space-y-4">
              <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-4">
                  <h3 className="font-bold text-gray-800 mb-3 flex items-center">
                      <ListChecks className="mr-2 text-teal-600" size={20}/> Việc cần làm chung
                  </h3>
                  ...
                  <div className="flex gap-2 mb-4">
                      <input 
                          ...
                          placeholder="Thêm việc (VD: Luộc rau, Lau sàn...)"
                  ...
                  <div className="space-y-2 max-h-[400px] overflow-y-auto pr-1">
                      {prepTasks.length === 0 && <p className="text-center text-gray-400 text-sm py-4">Chưa có công việc nào.</p>}
                      {prepTasks.map(task => (
                          <div key={task.id} className="flex items-center justify-between group p-2 hover:bg-gray-50 rounded-lg transition-colors border border-transparent hover:border-gray-100">
                              <div 
                              ...
                              >
                                  <div className={`shrink-0 transition-colors ${task.isCompleted ? 'text-green-500' : 'text-gray-300'}`}>
                                      {task.isCompleted ? <CheckCircle2 size={20} /> : <Square size={20} />}
                                  </div>
                                  <span className={`text-sm font-medium ${task.isCompleted ? 'text-gray-400 line-through' : 'text-gray-700'}`}>
                                      {task.task}
                                  </span>
                              </div>
                              <button 
          ...
          {/* RIGHT COLUMN: SERVING PREP (EXISTING) */}
                ...
                {preparationTotals.length > 0 && (
                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-3 md:p-4 overflow-hidden">
                        <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3 flex items-center gap-1">
                            <AlertCircle size={12} /> Tổng khối lượng cần làm (Theo đoàn)
                        </h3>
