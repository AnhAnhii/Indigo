...
export const ServingChecklist: React.FC = () => {
  ...
  const handleServeAll = (item: ServingItem) => {
      ...
      updateServingItem(selectedGroup.id, item.id, { servedQuantity: item.totalQuantity });
  }

  const handleGuestArrived = (e: React.MouseEvent, groupId: string) => {
      e.stopPropagation();
      ...
      return (
          <div className="space-y-6 animate-in slide-in-from-right duration-300 relative">
              <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                        <div className="flex items-center gap-2">
                            ...
                            {!isReadOnly && (
                                <button onClick={openEditGroupModal} className="text-gray-400 hover:text-teal-600 p-1"><Edit3 size={18} /></button>
                            )}
                        </div>
                        <div className="flex items-center text-gray-500 text-sm space-x-3 mt-1">
                            ...
                            {!selectedGroup.startTime && !isReadOnly && (
                                <button onClick={(e) => handleGuestArrived(e, selectedGroup.id)} className="bg-blue-600 text-white text-xs font-bold px-3 py-1 rounded-full animate-pulse hover:bg-blue-700 shadow-md flex items-center">
                                    <BellRing size={12} className="mr-1"/> Báo khách đến
                                </button>
                            )}
                  ...
                  <button onClick={(e) => handleDeleteGroupClick(e, selectedGroup.id)} className="p-2 text-red-500 hover:bg-red-50 rounded-full" title="Xóa đoàn này">
                      <Trash2 size={20} />
                  </button>
              </div>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-20">
                  <div className="p-4 bg-gray-50 border-b flex justify-between items-center">
                       ...
                       {!isReadOnly && (
                           <button onClick={openAddItemModal} className="text-teal-600 font-bold text-xs flex items-center hover:underline">
                  ...
                  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
                    <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
                        ...
                        <div className="p-4 border-t bg-gray-50 flex justify-between items-center">
                            <div>
                                {editingItem && (
                                    <button onClick={handleDeleteItem} className="text-red-500 p-2 hover:bg-red-50 rounded-lg transition-colors"><Trash2 size={18} /></button>
                                )}
                            </div>
              ...
              {deleteConfirmId && (
                  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
                      <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl p-6 text-center animate-in zoom-in duration-200">
                          ...
                          <p className="text-gray-500 text-sm mb-6">Bạn có chắc chắn muốn xóa đoàn này không? Hành động này không thể hoàn tác.</p>
                          <div className="flex gap-3">
                              <button onClick={() => setDeleteConfirmId(null)} className="flex-1 py-2.5 bg-gray-100 text-gray-700 font-bold rounded-xl hover:bg-gray-200 transition-colors">Hủy bỏ</button>
                              <button onClick={confirmDeleteGroup} className="flex-1 py-2.5 bg-red-600 text-white font-bold rounded-xl hover:bg-red-700 transition-colors shadow-md">Xóa ngay</button>
                          </div>
                      </div>
                  </div>
              )}
      ...
      <div className="space-y-3">
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar">
              <button 
                  ...
                  className={`px-4 py-2 rounded-full text-sm font-bold whitespace-nowrap transition-colors flex items-center gap-2 ${selectedZone === 'ALL' ? 'bg-teal-600 text-white' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50'}`}
              >
                  <LayoutGrid size={16} /> Tất cả ({getZoneCount('')})
              </button>
              {RESTAURANT_ZONES.map(zone => {
                  ...
                  return (
                      <button 
                          ...
                          className={`px-4 py-2 rounded-full text-sm font-bold whitespace-nowrap transition-colors border flex items-center gap-2 ${selectedZone === zone ? 'bg-teal-600 text-white border-teal-600' : 'bg-white border-gray-200 text-gray-600 hover:bg-gray-50'}`}
                      >
                          {zone}
      ...
      {viewMode === 'HISTORY' && (
          <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm flex items-center gap-4">
              ...
              <div className="relative">
                  ...
                  <input type="date" value={historyDate} onChange={(e) => setHistoryDate(e.target.value)} className="pl-10 pr-4 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-teal-500 outline-none" />
              </div>
          ...
          {viewMode === 'ACTIVE' && displayGroups.length === 0 && (
              <div className="col-span-full text-center py-16 bg-white rounded-2xl border border-dashed border-gray-300">
                  ...
                  <p className="text-gray-500 font-medium">
                      {searchTerm || selectedZone !== 'ALL' ? 'Không tìm thấy đoàn nào phù hợp bộ lọc.' : 'Hiện tại không có đoàn khách nào.'}
                  </p>
                  {!searchTerm && selectedZone === 'ALL' && <p className="text-sm text-gray-400">Bấm "Nhận đoàn mới" để bắt đầu.</p>}
              </div>
          )}

          {viewMode === 'HISTORY' && displayGroups.length === 0 && (
              <div className="col-span-full text-center py-16 bg-gray-50 rounded-2xl">
                  <p className="text-gray-500">Không tìm thấy đoàn khách nào hoàn thành ngày {historyDate}.</p>
              </div>
          )}
              ...
              return (
                  <div key={group.id} onClick={() => setSelectedGroupId(group.id)} className={`bg-white rounded-2xl p-6 shadow-sm border hover:shadow-md cursor-pointer transition-all group relative ${group.status === 'COMPLETED' ? 'border-green-200 bg-green-50/30' : 'border-gray-200'}`}>
                      ...
                      <div className="flex justify-between items-start mb-4">
                          ...
                          {group.status === 'COMPLETED' ? (
                               <span className="px-2 py-1 bg-green-100 text-green-700 text-xs font-bold rounded flex items-center"><CheckCircle2 size={12} className="mr-1"/> Completed</span>
                                ...
                                <button onClick={(e) => handleGuestArrived(e, group.id)} className="px-3 py-1 bg-blue-500 text-white text-xs font-bold rounded-full shadow-md animate-pulse hover:bg-blue-600 transition-colors z-20 flex items-center"><BellRing size={12} className="mr-1"/> Báo khách đến</button>
                          ) : (
                      ...
                      <div className="flex flex-wrap gap-2 items-center text-gray-500 text-xs mb-4">
                          ...
                          {group.tableSplit && <span className="text-[10px] bg-gray-100 px-1.5 py-0.5 rounded text-gray-600 font-mono border border-gray-200 truncate max-w-[100px]">{group.tableSplit}</span>}
                      </div>
                      ...
                      {group.status === 'COMPLETED' && (
                          <div className="mt-3 pt-3 border-t border-green-100 text-xs text-gray-600 bg-white/50 p-2 rounded">
                              ...
                              <p className="line-clamp-2"><span className="font-semibold">Thực đơn:</span> {group.items.map(i => `${i.name} (${i.servedQuantity})`).join(', ')}</p>
                          </div>
                      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
            <div className="bg-white rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200 max-h-[90vh] flex flex-col">
                ...
                <div className="p-4 border-t bg-gray-50 flex justify-end space-x-3 shrink-0">
                    ...
                    {step === 'REVIEW' && (
                        <button onClick={handleConfirmAllGroups} className="px-6 py-2 bg-teal-600 text-white font-bold text-sm rounded-lg hover:bg-teal-700 shadow-md flex items-center"><Save size={16} className="mr-2"/> Xác nhận & Tạo {detectedGroups.length} đoàn</button>
                    )}
                </div>
            </div>
        </div>
