...
import { Calendar as CalendarIcon, ChevronLeft, ChevronRight, Layers, Sunrise, Sunset, Moon, X, User, Clock, Plus, Briefcase, AlertTriangle, CheckCircle } from 'lucide-react';
...
export const ScheduleView: React.FC = () => {
  ...
  const isAdmin = currentUser?.role === EmployeeRole.MANAGER || 
                  currentUser?.role === EmployeeRole.DEV || 
                  currentUser?.role === EmployeeRole.DEPARTMENT_HEAD;
  ...
  const formatDateKey = (date: Date) => new Intl.DateTimeFormat('en-CA', { timeZone: 'Asia/Ho_Chi_Minh', year: 'numeric', month: '2-digit', day: '2-digit' }).format(date);

  const getShift = (empId: string, date: Date) => {
      const dateStr = formatDateKey(date);
      const schedule = schedules.find(s => s.employeeId === empId && s.date === dateStr);
      return schedule ? schedule.shiftCode : 'OFF';
  }
  ...
  const handleCellClick = (emp: Employee, date: Date) => {
      ...
      setEditingCell({ empId: emp.id, empName: emp.name, dateStr, currentShift });
      setIsModalOpen(true);
  }
  ...
  const getShiftConfig = (code: string) => settings.shiftConfigs?.find(s => s.code === code);
  ...
  const dailyStats = useMemo(() => {
      return weekDays.map(day => {
          ...
          const daySchedules = schedules.filter(s => s.date === dateStr && s.shiftCode !== 'OFF');
          const count = daySchedules.length;
          
          // Basic logic: Need at least 3 people, ideal 5-8, over 10 is crowded
          let status: 'LOW' | 'OK' | 'HIGH' = 'OK';
          if (count < 3) status = 'LOW';
          if (count > 10) status = 'HIGH';
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 flex-1 flex flex-col overflow-hidden relative">
            <div className="overflow-auto flex-1 no-scrollbar">
                <div className="min-w-[1000px]">
                    ...
                    {staffList.map((emp) => (
                        <div key={emp.id} className="grid grid-cols-[200px_repeat(7,_1fr)] group hover:bg-gray-50 transition-colors">
                            ...
                            {weekDays.map((day, idx) => {
                                ...
                                return (
                                    <div key={idx} className={`border-b border-r last:border-r-0 p-1.5 relative min-h-[90px]`}>
                                        <div onClick={() => handleCellClick(emp, day)} className={`w-full h-full rounded-xl flex flex-col items-center justify-center text-xs border transition-all relative overflow-hidden ${isAdmin ? 'cursor-pointer hover:shadow-md hover:scale-[1.02]' : ''} ${isOff ? 'bg-transparent border-transparent hover:bg-gray-100' : `${style.bg} ${style.border} ${style.text}`}`}>
                                            {isOff ? (isAdmin && <Plus className="text-gray-200 group-hover:text-gray-400" size={24} />) : (
                                                <>
                                                    <div className="flex flex-col items-center z-10 w-full px-1">
                                                        ...
                                                        <div className="flex items-center bg-white/60 px-2 py-0.5 rounded text-[10px] font-semibold backdrop-blur-sm whitespace-nowrap">
                                                            ...
                                                            {config ? (config.isSplitShift ? <span>{config.startTime}...{config.endTime}</span> : <span>{config.startTime} - {config.endTime}</span>) : <span>--:--</span>}
                                                        </div>
                                                    </div>
                    ...
                    {/* Footer Row (Stats) */}
                    <div className="grid grid-cols-[200px_repeat(7,_1fr)] border-t-2 border-gray-100 bg-gray-50">
                        <div className="p-3 font-bold text-xs uppercase text-gray-500 border-r flex items-center justify-end pr-4">Tổng nhân sự:</div>
                        {dailyStats.map((stat, idx) => (
                            <div key={idx} className="p-3 text-center border-r last:border-r-0">
                                <div className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold border ${stat.status === 'LOW' ? 'bg-red-100 text-red-700 border-red-200' : stat.status === 'HIGH' ? 'bg-yellow-100 text-yellow-700 border-yellow-200' : 'bg-green-100 text-green-700 border-green-200'}`}>
                                    {stat.status === 'LOW' && <AlertTriangle size={12} className="mr-1"/>}
                                    {stat.status === 'OK' && <CheckCircle size={12} className="mr-1"/>}
                                    {stat.count} người
                                </div>
                            </div>
                        ))}
                ...
                <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden animate-in zoom-in duration-200 flex flex-col max-h-[90vh]">
                    <div className="p-5 border-b bg-gray-50 flex justify-between items-center shrink-0">
                        <div className="flex items-center gap-3">
                            <div className="w-10 h-10 bg-teal-100 rounded-full flex items-center justify-center text-teal-700 font-bold"><User size={20}/></div>
                            <div><h3 className="font-bold text-gray-900 text-lg">{editingCell.empName}</h3><p className="text-xs text-gray-500 font-medium flex items-center"><CalendarIcon size={12} className="mr-1"/> {new Date(editingCell.dateStr).toLocaleDateString('vi-VN', {weekday:'long', day:'numeric', month:'long'})}</p></div>
                        </div>
                    ...
                    <div className="p-5 overflow-y-auto space-y-3 bg-gray-50/50">
                        ...
                        <button onClick={() => handleSelectShift('OFF')} className={`w-full p-4 rounded-xl border flex items-center justify-between transition-all group ${editingCell.currentShift === 'OFF' ? 'border-gray-400 bg-gray-200 ring-2 ring-gray-300' : 'bg-white border-gray-200 hover:border-gray-300 hover:shadow-sm'}`}>
                            <div className="flex items-center gap-3"><div className="w-10 h-10 rounded-lg bg-gray-100 flex items-center justify-center text-gray-500 group-hover:bg-gray-200"><X size={20}/></div><div className="text-left"><div className="font-bold text-gray-700">Nghỉ (OFF)</div><div className="text-xs text-gray-400">Không có lịch làm việc</div></div></div>
                        ...
                        {settings.shiftConfigs.map(shift => {
                            ...
                            return (
                                <button key={shift.code} onClick={() => handleSelectShift(shift.code)} className={`w-full p-3 rounded-xl border flex items-center justify-between transition-all group relative overflow-hidden ${isSelected ? `ring-2 ring-offset-2 ring-${style.text.split('-')[1]}-400 border-transparent shadow-md` : 'bg-white border-gray-200 hover:border-teal-300 hover:shadow-md'}`}>
                                    {isSelected && <div className={`absolute inset-0 opacity-10 ${style.bg}`}></div>}
                                    <div className="flex items-center gap-3 relative z-10"><div className={`w-12 h-12 rounded-lg flex items-center justify-center ${style.bg} ${style.text}`}><Icon size={20}/></div><div className="text-left"><div className="font-bold text-gray-900 text-base">{shift.name}</div><div className="text-xs text-gray-500 mt-0.5 flex items-center gap-1"><Clock size={12}/> {shift.isSplitShift ? <span className="font-mono text-purple-600 font-medium">{shift.startTime}-{shift.breakStart} & {shift.breakEnd}-{shift.endTime}</span> : <span className="font-mono">{shift.startTime} - {shift.endTime}</span>}</div></div></div>
